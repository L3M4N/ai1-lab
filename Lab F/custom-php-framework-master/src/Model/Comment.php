...
class Comment
{
    ...
    public function save(): void
    {
        $pdo = new \PDO(Config::get('db_dsn'), Config::get('db_user'), Config::get('db_pass'));
        if (! $this->getId()) {
            $sql = "INSERT INTO comment (author, content, post_id) VALUES (:author, :content, :post_id)";
            $statement = $pdo->prepare($sql);
            $statement->execute([
            ...
            $sql = "UPDATE comment SET author = :author, content = :content, post_id = :post_id WHERE id = :id";
            $statement = $pdo->prepare($sql);
            $statement->execute([
                ':author' => $this->getAuthor(),
                ':content' => $this->getContent(),
